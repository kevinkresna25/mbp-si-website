<x-public-layout>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <x-card-grid>
            ...
            <a wire:navigate href="{{ route('public.kegiatan.show', $item->id) }}" class="bg-white dark:bg-slate-800 rounded-3xl shadow-lg shadow-gray-200/50 dark:shadow-none border border-emerald-200 dark:border-emerald-500/20 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition duration-300 group flex flex-col h-full">
                <div class="aspect-video bg-gray-100 dark:bg-slate-700 overflow-hidden relative">
                    ...
                    @else
                    <div class="w-full h-full flex items-center justify-center bg-emerald-50 dark:bg-slate-700/50">
                        <svg class="w-12 h-12 text-emerald-300 dark:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    @endif
                    ...
                    <div class="absolute top-3 right-3 flex flex-col gap-2 items-end">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm text-gray-800 dark:text-gray-100 shadow-sm border border-emerald-100 dark:border-white/10">
                             {{ $item->jenis_label }}
                        </span>
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm bg-{{ $item->status_color }}-100 text-{{ $item->status_color }}-700 dark:bg-{{ $item->status_color }}-900/80 dark:text-{{ $item->status_color }}-300 backdrop-blur-sm border border-white/10">
                            {{ $item->status_label }}
                        </span>
                    </div>
                    ...
                    <div class="absolute bottom-3 left-3 text-white">
                        <div class="flex items-center gap-2">
                             <div class="bg-white/20 backdrop-blur-md rounded-xl px-3 py-1.5 flex flex-col items-center border border-white/30 shadow-lg">
                                 <span class="text-[10px] uppercase font-bold tracking-wider">{{ $item->tanggal->translatedFormat('M') }}</span>
                ...
                <div class="p-6 flex flex-col flex-grow">
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-3 line-clamp-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition">{{ $item->nama_kegiatan }}</h2>
                    
                    <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400 mt-auto pt-4 border-t border-emerald-100 dark:border-white/5">
                        @if($item->lokasi)
                        <div class="flex items-start gap-2">
                            ...
                            <span class="line-clamp-1 font-medium">{{ $item->lokasi }}</span>
                        </div>
                        @endif
                        @if($item->ustadz)
                        <div class="flex items-start gap-2">
                            <svg class="w-4 h-4 mt-0.5 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            <span class="line-clamp-1 font-medium">{{ $item->ustadz }}</span>
                        </div>
                        @endif
                    </div>
                </div>
